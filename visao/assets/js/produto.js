$(document).ready(function (e) {
  ...
  if (
    recebeUrlAtualProdutos ===
    "http://localhost/vendinha/visao/index.php?pagina=consulta_produto"
  ) {
    listarProdutos();
    ...
    $("#recebe-mensagem-campo-falha-carregar-dados-produto").hide();
  } else if (
    recebeUrlAtualProdutos ===
    "http://localhost/vendinha/visao/index.php?pagina=cadastrar_produto"
  ) {
...
function listarProdutos() {
  $.ajax({
    ...
    error: function (xhr, status, error) {
      $("#recebe-mensagem-campo-falha-buscar-produto").html(
...
$("#cadastra-produto").click(function (e) {
  ...
  if (recebeNomeProduto != "" && recebeValorProduto != "") {
    ...
    let temporariostring = recebeVProdutoFinalNumerico.replace(
      /\.(?=[^.]*$)/,
      "TEMP"
    );
    ...
    dados_cadastro_produto.append(
      "valor_produto_numerico",
      valor_produto_numerico
    );
    dados_cadastro_produto.append(
      "processo_produto",
    ...
    $.ajax({
      ...
      success: function (resultado) {
        ...
        if (resultado > 0) {
          ...
          $("#recebe-mensagem-cadastro-realizado-produto").fadeOut(4000);
        } else {
          $("#recebe-mensagem-campo-falha-cadastro-produto").html(
            "Falha ao cadastrar produto:" + resultado
          );
...
function carrega_dados_produto_alteracao(recebeCodigoProdutoAlteracao, e) {
  ...
  if (recebeCodigoProdutoAlteracao) {
    $.ajax({
      ...
      error: function (xhr, status, error) {
        $("#recebe-mensagem-campo-falha-carregar-dados-produto").html(
          "Falha ao carregar produto:" + error
        );
        $("#recebe-mensagem-campo-falha-carregar-dados-produto").show();
        $("#recebe-mensagem-campo-falha-carregar-dados-produto").fadeOut(4000);
      },
    });
  }
}
...
$("#alterar-produto").click(function (e) {
  ...
  if (recebeNomeProdutoAlterar != "" && recebeValorProdutoAlterar != "") {
  ...
  if (recebeRespostaExcluirProdutoEspecifico) {
    $.ajax({
      ...
      success: function (retorno) {
        ...
        if (retorno) {
          $("#recebe-mensagem-exclusao-realizado-produto").html(
